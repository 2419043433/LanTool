    ...
    public boolean processCommand(CommandId id, Params param, Params result)
    {
        ...
        switch (id)
        {
            ...
            case OnFileTransferRequest:
                {
                    InetSocketAddress remoteAddress = (InetSocketAddress) param.get(ParamKeys.Address);
                    ...
                    int option = JOptionPane.showConfirmDialog(null, sb.toString(), "文件接收提示！", JOptionPane.YES_NO_OPTION);
                    mMessageHandler.handleMessage(option == JOptionPane.YES_OPTION ? MessageId.OnAcceptFileTransferRequest : MessageId.OnDenyFileTransferRequest, param, result);
                }
                break;
